...
"""
This code runs the warmup training of the RALMU model on a dataset based on the metadata.csv file provided
We can train by optimizing the loss for every layer (`return_layers`) or for the global output only
...
def main(num_workers, n_iter, lr, epochs, batch, length, subset, split, clip):
    ...
    train_set   = utils.LocalDataset(train_data, use_midi=False, fixed_length=length, subset=subset, dtype=dtype)
    valid_set   = utils.LocalDataset(valid_data, use_midi=False, fixed_length=length, subset=subset, dtype=dtype)
    ...
    print(f"Clip = {clip}")
    ralmu = models.RALMU(l=88, beta=1, W_path=W_path, hidden=16, n_iter=n_iter, n_init_steps=10, shared=False, warmup=True, return_layers=True, aw_2d=aw2d, clip_H=clip, dtype=dtype)
    ralmu = ralmu.to(dev)
    ...
    # losses, W_hat, H_hat = utils.warmup_train(ralmu, epochs, train_loader, optimizer, dev, save_name="15_iter_non_shared_warmup", load_check="/home/ids/edabier/AMT/Unrolled-NMF/models/checkpoint.pt", use_wandb=True)
    losses, W_hat, H_hat = utils.warmup_train(ralmu, epochs, train_loader, optimizer, dev, train_layers=True, save_name="non_shared_layers_warmup", load_check=None, use_wandb=True)
